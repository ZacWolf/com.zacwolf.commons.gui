package com.zacwolf.commons.gui.printhelpers;
...
import com.zacwolf.commons.gui.PropertyUtilities;
...
public class PrintHelper implements Printable{
   ...
   public PrintHelper(Component target, Properties user)  {
      this.target = target;
      this.user = user;
      ...
      PrintService ps = pj.getPrintService();
        if (ps == null) throw new RuntimeException("No Print Service Found");
   ...
   {
      PrintPreview pp = new PrintPreview(user){
      ...
      if (media instanceof MediaSizeName)
      {
         ...
         paper.setImageableArea(72 * ma.getX(ma.INCH), 72 * ma.getY(ma.INCH), 72 * ma.getWidth(ma.INCH), 72 * ma.getHeight(ma.INCH));
         pf.setPaper(paper);
      }
